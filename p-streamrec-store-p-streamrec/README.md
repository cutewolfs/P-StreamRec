...
To set Chaturbate credentials, a `PASSWORD`, or `AUTO_RECORD_USERS`, edit the `environment:` section of `docker-compose.yml` in the installed app directory on your Umbrel (`~/umbrel/app-data/p-streamrec-store-p-streamrec/`) and restart the app from the UI.
